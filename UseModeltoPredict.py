...
stepsS = 1, 5, 10, 20
R_predictions = 0.8, 1.0, 1.2, 1.4, 1.6

for steps in stepsS:
    for R_prediction in R_predictions:

        model = tf.keras.models.load_model('/home/grouptan/Documents/yudian/data/linebyline1092singlepitchtrainingreducedlengthSteps'+str(steps)+'/tf_model')

        import os
        df_actual = pd.DataFrame([])
        df_pred = pd.DataFrame([])
        R_prediction = round(R_prediction, 1)
        # Specify the path for the new folder
        folder_path = '/home/grouptan/Documents/yudian/data/linebyline1092predictionreducedlengthPitch'+str(R_prediction)+'Steps'+str(steps)  # Replace with the desired path
        ...
        df_result.to_csv(folder_path+'/df_result_steps'+str(steps)+'_pitch'+str(R_prediction)+'.csv')
        df_pred.to_csv(folder_path+'/df_pred_steps'+str(steps)+'_pitch'+str(R_prediction)+'.csv')
        df_actual.to_csv(folder_path+'/df_actual_steps'+str(steps)+'_pitch'+str(R_prediction)+'.csv')
        ...
        plt.savefig(folder_path+'/Actual_steps'+str(steps)+'.jpg', format='jpg')
        plt.show()
        plt.close()
        ...
        ax.xaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))
        ax.yaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))
        plt.savefig(folder_path+'/Predicted_steps'+str(steps)+'.jpg', format='jpg')
        plt.show()
        plt.close()
