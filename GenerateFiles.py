...
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.legend(["0.6µm", "0.8µm","1.0µm","1.2µm","1.4µm","1.6µm",], prop={'weight': 'bold','size': 35}, loc = "best")
plt.show()
plt.close()
...
n = 387
print(n/387*60)
url = 'https://raw.githubusercontent.com/yd145763/DifferentPitchDifferentStepsML/main/df_actual_steps'+str(steps)+'_pitch'+str(pitch)+'.csv'
df1 = pd.read_csv(url)
df1 = df1.iloc[:, 1:]
actual_e = df1['e'+str(n)]
x = df1.iloc[:, -1]

url = 'https://raw.githubusercontent.com/yd145763/DifferentPitchDifferentStepsML/main/df_pred_steps'+str(steps)+'_pitch'+str(pitch)+'.csv'
df1 = pd.read_csv(url)
df1 = df1.iloc[:, 1:]
pred_e = df1['e'+str(n)]
...
plt.legend(["Original E-field", "Predicted E-field"], prop={'weight': 'bold','size': 35}, loc = "best")
plt.show()
plt.close()
...
steps = 1
#training - validation
url = 'https://raw.githubusercontent.com/yd145763/DifferentPitchDifferentStepsML/main/df_training_step'+str(steps)+'.csv'
df_training = pd.read_csv(url)
df_training = df_training.iloc[:, 1:]

url = 'https://raw.githubusercontent.com/yd145763/DifferentPitchDifferentStepsML/main/df_validation_step'+str(steps)+'.csv'
...
for steps, marker in zip(stepsS, markerS):
    url_actual = 'https://raw.githubusercontent.com/yd145763/DifferentPitchDifferentStepsML/main/df_actual_steps'+str(steps)+'_pitch'+str(pitch)+'.csv'
    df1_actual = pd.read_csv(url_actual)
    df1_actual = df1_actual.iloc[:, 1:]
    
    url_pred = 'https://raw.githubusercontent.com/yd145763/DifferentPitchDifferentStepsML/main/df_pred_steps'+str(steps)+'_pitch'+str(pitch)+'.csv'
